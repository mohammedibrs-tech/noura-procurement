...
async function callClaude({ system, prompt, useWebSearch = false, timeoutMs = 35000 }) {
  const body = {
  ...
  try {
    ...
    if (err.name === 'AbortError') throw new Error(`Claude API timed out after ${timeoutMs}ms`);
    throw err;
  } finally {
    clearTimeout(timer);
  }
...
ابحث بالإنترنت عن 3 إلى 5 موردين بديلين حقيقيين بالسوق السعودي لنفس نوع الخدمة. **رقم الهاتف/الجوال إلزامي لكل مورد** — ابحث عنه تحديداً بموقع الشركة الرسمي، صفحة "اتصل بنا"، Google Maps/Google Business، أو منصات الأعمال (مثل الدليل السعودي، بيانات.سا). لا تقترح مورداً إلا إذا لقيت له رقم هاتف حقيقي. اذكر لكل واحد: اسمه الحقيقي، رقم الهاتف (إلزامي)، موقعه الإلكتروني إن وجد، بريده الإلكتروني إن كان متاحاً علناً، وسبب ترشيحه. اذكر أيضاً أي تطورات حديثة بهذا السوق.

مهم جداً: أجب بصيغة JSON صِرف فقط، بدون أي نص قبله أو بعده. لا تدرج أي مورد بدون رقم هاتف فعلي بالشكل التالي بالضبط:
{"alternatives":[{"name":"...","phone":"...","website":"...","email":"...","reason":"..."}],"market_note":"..."}`;

  try {
    const raw = await callClaude({
      system:
        'أنت محلل مشتريات خبير يبحث بالإنترنت فعلياً ويرجّع نتائج حقيقية ودقيقة بصيغة JSON فقط، بدون أي اختلاق لمعلومات غير مؤكدة. رقم الهاتف شرط أساسي — أي مورد بدون رقم هاتف حقيقي لا يُدرج بالنتيجة إطلاقاً.',
      prompt,
      useWebSearch: true,
    });
    const parsed = extractJSON(raw);
    if (parsed) {
      // فلترة إضافية من جهتنا: نستبعد أي مورد ما جاب له رقم هاتف حتى لو Claude غفل عن الشرط
      const withPhone = (parsed.alternatives || []).filter((a) => a.phone && String(a.phone).trim());
      return {
        kind: 'contract',
        suggestion: {
          ...
          days_left: days,
          alternatives: withPhone,
          error: withPhone.length === 0 ? 'لم يُعثر على موردين برقم هاتف موثّق بهذا التشغيل' : undefined,
        },
...
2) مورد واحد إضافي حقيقي بالسوق السعودي (غير الموردين المذكورين أعلاه) يستحق دعوته لتقديم عرض. **رقم الهاتف/الجوال إلزامي** — ابحث عنه بموقع الشركة، Google Maps/Google Business، أو منصات الأعمال. لا تقترح مورداً بدون رقم هاتف حقيقي. اذكر أيضاً موقعه الإلكتروني وبريده الإلكتروني إن كانا متاحين علناً.

مهم جداً: أجب بصيغة JSON صِرف فقط، بدون أي نص قبله أو بعده، بالشكل التالي بالضبط:
{"market_price_range":"...","suggested_vendor":{"name":"...","phone":"...","website":"...","email":"..."}}`;

  try {
    const raw = await callClaude({
      system:
        'أنت محلل مشتريات خبير يبحث بالإنترنت فعلياً ويرجّع نتائج حقيقية ودقيقة بصيغة JSON فقط، بدون أي اختلاق لمعلومات غير مؤكدة. رقم الهاتف شرط أساسي للمورد المقترح — لا تقترح مورداً بدون رقم هاتف حقيقي.',
      prompt,
      useWebSearch: true,
    });
    const parsed = extractJSON(raw);
    // نستبعد المورد المقترح لو ما فيه رقم هاتف فعلي، حتى لو Claude غفل عن الشرط
    let suggestedVendor = (parsed && parsed.suggested_vendor) || null;
    if (suggestedVendor && !(suggestedVendor.phone && String(suggestedVendor.phone).trim())) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // أحدث طلبين عروض أسعار مفتوحة فقط — لضبط الوقت والتكلفة (قلّلنا من 3 بسبب انتهاء المهلة)
    const openRFQs = rfqs
      .slice()
